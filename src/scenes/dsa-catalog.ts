...
// The "top data structures" reference poster — 12 structures, each drawn the way
// you'd sketch it on a whiteboard (boxed sequences for the linear ones; node-and-
// link diagrams for the trees/graphs). Companion to `dsa.ts` (the taxonomy): that
// scene says WHERE each structure sits in the family, this one says what each one
// LOOKS like. Calm filled style throughout (DESIGN.md) — NOT the neon reference:
// circles are muted FILLED disks, not glowing outlines.
...
// A filled value chip (array / queue / list cell, hashmap bucket).
...
// Each is a small graph of `circle` vertices placed on a 7-/3-wide grid; the links
// are scene edges (below). Vertical scene direction puts parent-on-top → child-below.
...
// =============== POSTER GRID ===============
// 6 cols × 5 rows. Halves (3+3), thirds (2+2+2), and one full-width band:
//
//   ARRAY (½)            QUEUE (½)
//   MATRIX  ·  STACK  ·  TREE
//   LINKED LIST (full)
//   HASHMAP ·  BST    ·  HEAP
//   TRIE    ·  GRAPH  ·  UNION FIND

const poster = group(
  'dsa-catalog-poster',
  wgrid(
    { cols: [1, 1, 1, 1, 1, 1], rows: [1, 1.25, 0.8, 1.25, 1.25], gap: 0.22, padding: 0.25 },
    [
      { node: arrayBox, at: [0, 0, 3, 1] },
      { node: queueBox, at: [3, 0, 3, 1] },

      { node: matrixBox, at: [0, 1, 2, 1] },
      { node: stackBox, at: [2, 1, 2, 1] },
      { node: treeBox, at: [4, 1, 2, 1] },

      { node: linkedListBox, at: [0, 2, 6, 1] },

      { node: hashMapBox, at: [0, 3, 2, 1] },
      { node: bstBox, at: [2, 3, 2, 1] },
      { node: heapBox, at: [4, 3, 2, 1] },

      { node: trieBox, at: [0, 4, 2, 1] },
      { node: graphBox, at: [2, 4, 2, 1] },
      { node: unionFindBox, at: [4, 4, 2, 1] },
    ],
  ),
)

export const dsaCatalog: SceneSpec = {
  id: 'dsa-catalog',
  topic: 'dsa',
  title: 'DSA — Catalog',
  subtitle: 'The top data structures, each at a glance',
  // The poster is near-square (6 cols × ~5.5 weighted rows); a 4:3 canvas keeps the
  // tree/graph boxes tall enough while fitView centers it in the 16:9 frame.
