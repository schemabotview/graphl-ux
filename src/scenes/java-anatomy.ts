...
// A filled CHIP whose text IS the concept (graphl-ux `term`). Everything in this
// scene is a conceptual token (a keyword, a type, an operator), not a system
// component, so chips read better than icon glyphs here.
...
const modelRow = container(
  ...
  wgrid({ cols: [1, 1], rows: [1, 1, 1], gap: 0.3, padding: 0.4 }, [
    { node: chip('ja-oop-animal', 'abstract class  Animal', PURPLE), at: [0, 0] },
    { node: chip('ja-oop-walker', 'interface  Walker', PURPLE), at: [1, 0] },
    { node: chip('ja-oop-mammal', 'class  Mammal   extends Animal   implements Walker', PURPLE), at: [0, 1, 2, 1] },
    { node: chip('ja-oop-dog', 'class  Dog   extends Mammal', PURPLE), at: [0, 2, 2, 1] },
  ]),
)
...
const kind = container(
  ...
  wgrid({ cols: [1], rows: [1, 1, 1], gap: 0.15, padding: 0.3 }, [
    { node: chip('ja-bind-final', 'final', GRAY), at: [0, 0] },
    { node: chip('ja-bind-static', 'static', GRAY), at: [0, 1] },
    { node: chip('ja-bind-private', 'private', GRAY), at: [0, 2] },
  ]),
)
...
const typePrimitives = container(
  ...
  wgrid({ cols: [1], rows: [1, 1, 1, 1, 1, 1], gap: 0.1, padding: 0.2 }, [
    { node: chip('ja-prim-int', 'int', GRAY), at: [0, 0] },
    { node: chip('ja-prim-long', 'long', GRAY), at: [0, 1] },
    { node: chip('ja-prim-double', 'double', GRAY), at: [0, 2] },
    { node: chip('ja-prim-float', 'float', GRAY), at: [0, 3] },
    { node: chip('ja-prim-boolean', 'boolean', GRAY), at: [0, 4] },
    { node: chip('ja-prim-char', 'char', GRAY), at: [0, 5] },
  ]),
)
...
const typeCollections = container(
  ...
  wgrid({ cols: [1], rows: [1, 1, 1, 1], gap: 0.1, padding: 0.2 }, [
    { node: chip('ja-coll-list', 'List', GRAY), at: [0, 0] },
    { node: chip('ja-coll-deque', 'Deque', GRAY), at: [0, 1] },
    { node: chip('ja-coll-map', 'Map', GRAY), at: [0, 2] },
    { node: chip('ja-coll-set', 'Set', GRAY), at: [0, 3] },
  ]),
)
...
const typeOop = container(
  ...
  wgrid({ cols: [1], rows: [1, 1, 1, 1, 1], gap: 0.1, padding: 0.2 }, [
    { node: chip('ja-class', 'class', GRAY), at: [0, 0] },
    { node: chip('ja-interface', 'interface', GRAY), at: [0, 1] },
    { node: chip('ja-abstract', 'abstract', GRAY), at: [0, 2] },
    { node: chip('ja-record', 'record', GRAY), at: [0, 3] },
    { node: chip('ja-enum', 'enum', GRAY), at: [0, 4] },
  ]),
)
...
const typeMeta = container(
  ...
  wgrid({ cols: [1], rows: [1, 1, 1, 1], gap: 0.1, padding: 0.2 }, [
    { node: chip('ja-generic', 'Generic<A>', GRAY), at: [0, 0] },
    { node: chip('ja-sealed', 'sealed', GRAY), at: [0, 1] },
    { node: chip('ja-array', 'Array', GRAY), at: [0, 2] },
    { node: chip('ja-optional', 'Optional', GRAY), at: [0, 3] },
  ]),
)
...
const typeShapes = container(
  ...
  wgrid({ cols: [0.6, 1, 1, 1.1, 1.1], rows: [1], gap: 0.2, padding: 0.3 }, [
    { node: chip('ja-type-var', 'var', GRAY), at: [0, 0] },
...
const value = container(
  ...
  wgrid({ cols: [1], rows: [1, 1, 1, 1], gap: 0.18, padding: 0.3 }, [
    { node: chip('ja-value-primitive', "Primitive    42  true  'a'", GRAY), at: [0, 0] },
    { node: chip('ja-value-object', 'Object       new Person()', GRAY), at: [0, 1] },
    { node: chip('ja-value-collection', 'Collection   List.of()  Map.of()', GRAY), at: [0, 2] },
    { node: chip('ja-value-defbody', 'Method body   n + 1   { ... }', GRAY), at: [0, 3] },
  ]),
)
...
const initializeRow = container(
  ...
  wgrid({ cols: [1.1, 4.2, 0.9, 0.4, 1.5], rows: [1], gap: 0.3, padding: 0.4 }, [
    ...
    { node: chip('ja-name', 'Name  s', BLUE), at: [2, 0] },
    { node: chip('ja-equals', '=', BLUE), at: [3, 0] },
    { node: value, at: [4, 0] },
  ]),
)
...
const control = container(
  ...
  wgrid({ cols: [1], rows: [1, 1, 1, 1], gap: 0.15, padding: 0.3 }, [
    { node: chip('ja-ctrl-if', 'if/else', GRAY), at: [0, 0] },
    { node: chip('ja-ctrl-tern', 'ternary', GRAY), at: [0, 1] },
    { node: chip('ja-ctrl-switch', 'switch', GRAY), at: [0, 2] },
    { node: chip('ja-ctrl-guard', 'guard', GRAY), at: [0, 3] },
  ]),
)
...
const patternMatch = container(
  ...
  wgrid({ cols: [1], rows: [1, 1], gap: 0.15, padding: 0.3 }, [
    { node: chip('ja-pat-match', 'case', GRAY), at: [0, 0] },
    { node: chip('ja-pat-record', 'deconstruct', GRAY), at: [0, 1] },
  ]),
)
...
const loops = container(
  ...
  wgrid({ cols: [1, 1], rows: [1, 1, 1], gap: 0.15, padding: 0.3 }, [
    { node: chip('ja-loop-for', 'for', GRAY), at: [0, 0] },
    { node: chip('ja-loop-foreach', 'for-each', GRAY), at: [1, 0] },
    { node: chip('ja-loop-while', 'while', GRAY), at: [0, 1] },
    { node: chip('ja-loop-dowhile', 'do-while', GRAY), at: [1, 1] },
    { node: chip('ja-loop-break', 'break', GRAY), at: [0, 2] },
    { node: chip('ja-loop-continue', 'continue', GRAY), at: [1, 2] },
  ]),
)
...
const methods = container(
  ...
  wgrid({ cols: [1], rows: [1, 1], gap: 0.15, padding: 0.3 }, [
    { node: chip('ja-method-call', 'x.foo()', GRAY), at: [0, 0] },
    { node: chip('ja-method-def', 'method in class', GRAY), at: [0, 1] },
  ]),
)
...
const lambdas = container(
  ...
  wgrid({ cols: [1], rows: [1, 1, 1], gap: 0.15, padding: 0.3 }, [
    { node: chip('ja-fn-lambda', 'lambda', GRAY), at: [0, 0] },
    { node: chip('ja-fn-methodref', 'method ref', GRAY), at: [0, 1] },
    { node: chip('ja-fn-iface', '@FunctionalInterface', GRAY), at: [0, 2] },
  ]),
)
...
const streamOps = container(
  ...
  wgrid({ cols: [1, 1], rows: [1, 1, 1], gap: 0.15, padding: 0.3 }, [
    { node: chip('ja-op-map', 'map', GRAY), at: [0, 0] },
    { node: chip('ja-op-filter', 'filter', GRAY), at: [1, 0] },
    { node: chip('ja-op-flatmap', 'flatMap', GRAY), at: [0, 1] },
    { node: chip('ja-op-reduce', 'reduce', GRAY), at: [1, 1] },
    { node: chip('ja-op-collect', 'collect', GRAY), at: [0, 2] },
    { node: chip('ja-op-groupby', 'groupingBy', GRAY), at: [1, 2] },
  ]),
)
...
const content = group(
  'ja-content',
  wgrid({ cols: [1], rows: [4, 5, 5, 2], gap: 0.5, padding: 0 }, [
...
const memory = container(
  ...
  wgrid({ cols: [1], rows: [1, 1.4, 1, 1], gap: 0.4, padding: 0.4 }, [
    { node: chip('ja-stack', 'Stack frame  (refs)', TEAL), at: [0, 0] },
    { node: chip('ja-heap', 'Heap  (objects)', TEAL), at: [0, 1] },
    { node: chip('ja-immutable', 'Immutable view  (List.of)', TEAL), at: [0, 2] },
    { node: chip('ja-method-area', 'Method Area  (bytecode)', TEAL), at: [0, 3] },
  ]),
)
...
export const javaAnatomy: SceneSpec = {
  ...
  // ~4:3 canvas (source was 24×18) so the grid renders with roughly square cells.
